...
import iccta.Ic3Data.Application.Component.IntentFilter;
import iccta.Ic3Data.Attribute;

/**
 * @author & @owner - Developers of (Flowdroid + ICCTA)  
 */
public class App {
...
class Intent {
	...
	public String getDataPath() {
		//System.out.println("App dataPath -> " + dataPath);
		return dataPath;
	}
		...
		if (data.contains("://")) {
			if (dataScheme == null) {
				...
				if (dataScheme.contains(".*"))
					dataScheme = null;
				//System.out.println("URI dataScheme -> " + dataScheme);
			}
			if (!data.isEmpty()) {
				if (dataHost == null) {
					if (data.contains("/"))
					...
					if (dataHost.contains(".*"))
						dataHost = null;
					//System.out.println("URI dataHost -> " + dataHost);
				}
				if (dataPath == null && !data.isEmpty()) {
					...
					if (dataPath.contains(".*"))
						dataPath = null;
					//System.out.println("URI dataPath -> " + dataPath);
				}
			}
		}
	}
		...
		for (Component component : list) {
			...
			if (isImplicit) {
				...
				for (IntentFilter filter : component.getIntentFiltersList()) {
					...
					//System.out.println("IntentFilter -> " +  filter);
					...
					for (Attribute attribute : filter.getAttributesList()) {
						//System.out.println("IntentFilter attribute -> " +  attribute);
						...
						case URI:
							...
							setData(attribute.getValue(0));
							hosts.add(getDataHost());
							schemes.add(getDataScheme());
							paths.add(getDataPath());
							break;
						case HOST:
							if (attribute.getValueCount() != 1)
								throw new RuntimeException("Valuecount != 1");
							hosts.add(attribute.getValueList().get(0));
							//System.out.println("IntentFilter HOST -> " +  attribute.getValueList().get(0));
							break;
						case SCHEME:
							if (attribute.getValueCount() != 1)
								throw new RuntimeException("Valuecount != 1");
							schemes.add(attribute.getValueList().get(0));
							//System.out.println("IntentFilter schemes -> " +  attribute.getValueList().get(0));
							break;
						case PORT:
							if (attribute.getValueCount() != 1)
								throw new RuntimeException("Valuecount != 1");
						...
						case PATH:
							...
							//System.out.println("IntentFilter PATH -> " +  attribute.getValueList().get(0));
							break;
						case AUTHORITY:
							if (attribute.getValueCount() != 1)
								throw new RuntimeException("Valuecount != 1");
					...
					if (!paths.isEmpty()) {
						for (String input : paths) {
							//System.out.println("IntentFilter PATH matchesURI input -> " +  input);
							String regex = ("\\Q" + input + "\\E").replace("*", "\\E.*\\Q");
							//System.out.println("IntentFilter PATH matchesURI regex -> " +  regex);
							String s = getDataPath();
							//System.out.println("IntentFilter PATH matchesURI getDataPath() -> " +  getDataPath());
							if (s == null)
								s = "";
							matchesURI |= s.matches(regex);
							//System.out.println("IntentFilter PATH matchesURI s -> " +  s);
							//System.out.println("IntentFilter PATH matchesURI -> " +  matchesURI);
						}
					}
					if (!hosts.isEmpty())
					...
					if (containsURI && getType() == null) {
						...
						//System.out.println("IntentFilter PATH passedData Val -> " +  passedData);
					}
					 ...
					 */
					//					System.out.println("IntentFilter PATH passedActionPart -> " +  passedActionPart);
					//					System.out.println("IntentFilter PATH passedCategory -> " +  passedCategory);
					//					System.out.println("IntentFilter PATH passedData -> " +  passedData);
					if (passedActionPart && passedCategory && passedData) {
						// This intent filter succeeded
						results.add(component);
						//System.out.println("IntentFilter passedActionPart && passedCategory && passedData -> " +  component);
						break;
					}
				}
			}
